...
def indexesFromSentence(voc, sentence):
    return [voc.word2index[word] for word in sentence.split(' ')] + [vocabs.EOS_token]


def zeroPadding(l, fillvalue=vocabs.PAD_token):
    return list(itertools.zip_longest(*l, fillvalue=fillvalue))

def binaryMatrix(l, value=vocabs.PAD_token):
    m = []
    for i, seq in enumerate(l):
        m.append([])
        for token in seq:
            if token == vocabs.PAD_token:
                m[i].append(0)
            else:
                m[i].append(1)
    return m
